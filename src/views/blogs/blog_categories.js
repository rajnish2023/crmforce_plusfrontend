...
const BlogCategory = () => {
  ...
  const toggleModal = () => {
    ...
    if (modalVisible) {
      setNewCategory({ title: '', slug: '', status: 'Active' });  // Reset form when closing
  ...
  const handleCategoryUpdate = async () => {
    ...
    try {
      await updateCategory(editCategory._id, newCategory);
      loadCategories(); // Reload categories after update

      setEditCategory(null);
      setNewCategory({ title: '', slug: '', status: 'Active' });
      setModalVisible(false);
      setServerError('');  // Clear any previous server errors
    } catch (error) {
